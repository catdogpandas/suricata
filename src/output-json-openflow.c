...
{
    OPENFLOWTransaction *openflowtx = tx;
    LogOPENFLOWLogThread *thread = thread_data;

    SCLogNotice("Logging openflow transaction %"PRIu64".", openflowtx->tx_id);
    ...
    if (openflowtx->request_buffer != NULL) {
        jb_set_string_from_bytes(js, "request", openflowtx->request_buffer,
                openflowtx->request_buffer_len);
    }

    /* Log the response buffer. */
    if (openflowtx->response_buffer != NULL) {
        jb_set_string_from_bytes(js, "response", openflowtx->response_buffer,
                openflowtx->response_buffer_len);
    }
...
void JsonOPENFLOWLogRegister(void)
{
    ...
    OutputRegisterTxSubModule(LOGGER_JSON_OPENFLOW, "eve-log", "JsonOPENFLOWLog",
        "eve-log.openflow", OutputOPENFLOWLogInitSub, ALPROTO_OPENFLOW,
        JsonOPENFLOWLogger, JsonOPENFLOWLogThreadInit,
        JsonOPENFLOWLogThreadDeinit, NULL);
